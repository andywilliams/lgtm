...
standards
  ...
  .addHelpText('after', "\nExit codes: 4 = written, but this repo's ESLint was NOT run over it (no local binary, a preview --out, a timeout, could not spawn) \u2014 run the lint yourself before committing \u00b7 3 = written, but this repo's ESLint cannot lint it at all (which may predate this run) \u00b7 1 = the command failed and nothing was written \u00b7 0 = written, and either ESLint passed it or reported findings, or this repo has no ESLint to run (none configured, --no-eslint). 0 is not a promise the lint is clean.")
